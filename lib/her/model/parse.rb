    ...
    module Parse
      ...
      module ClassMethods
        ...
        def embeded_params(attributes)
          (associations[:has_many] + associations[:has_one]).select { |a| attributes.include?(a[:data_key])}.compact.inject({}) do |hash, association|
            # avoid hash is nil
            hash ||= {}
            params = {}
            if attributes[association[:data_key]].respond_to?(:map)
